...
function getReferrerSource(referrer) {
    ...
    try {
        ...
        const socialDomains = ['facebook.com', 'twitter.com', 'instagram.com', 'linkedin.com', 'tiktok.com', 'pinterest.com', 'reddit.com', 'youtube.com'];
...
function generateVisitorId() {
    const fingerprint = [navigator.userAgent, navigator.language, screen.width + 'x' + screen.height, new Date().getTimezoneOffset(), window.location.hostname].join('|');
    let hash = 0;
...
async function getRobustIP() {
    const ipServices = [
        ...
        { url: 'https://ipapi.co/json/', key: 'ip' } // fallback
    ];
...
async function getRobustLocation(ip) {
    try {
    ...
    if (window.location.protocol === 'http:') {
        try {
            const response = await fetch(`http://ip-api.com/json/${ip || ''}?fields=status,country,countryCode,city,regionName,lat,lon,isp,org,timezone`);
            const data = await response.json();
...
async function trackVisitor() {
    try {
        ...
        const ip = await getRobustIP();
        const locationData = await getRobustLocation(ip);

        if (locationData) {
            visitorData.ip = locationData.ip || ip;
            visitorData.country = locationData.country || 'Unknown';
            visitorData.countryCode = locationData.countryCode || 'XX';
            visitorData.city = locationData.city || 'Unknown';
            visitorData.region = locationData.region || '';
            visitorData.latitude = locationData.latitude || 0;
            visitorData.longitude = locationData.longitude || 0;
            visitorData.isp = locationData.isp || 'Unknown';
            
            if (visitorData.countryCode === 'MA' || visitorData.country === 'MA') {
                visitorData.country = 'Morocco';
                visitorData.countryCode = 'MA';
            }
        } else {
            visitorData.ip = ip || 'Unknown';
        ...
        hasTracked = true;
        markAsVisited();
        console.log('Visitor tracked successfully from:', visitorData.country);
